	...
	for instance in Project.objects.all():
		if instance.project_name == request.session['project_name']:
			...
			if 'submit' in request.POST:
				...
				instance.estimate_yield = request.POST.get('yield')
				instance.save()
				context = {
					"state":"Data is saved."
				}
	...
	if 'close' in request.POST:
		...
		for instance in Iteration.objects.all():
			if(instance.project_name == request.session['project_name'] and
				...
				count=0
				if request.session['phase_name'] == 'Elaboration Phase' and request.session['iteration_name'] == 'Iteration 1':
					for instance2 in Defect.objects.all():
						if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Inception Phase' and instance2.iteration_name == 'Iteration 1':
							count=count+1
				if request.session['phase_name'] == 'Elaboration Phase' and request.session['iteration_name'] == 'Iteration 2':
					for instance2 in Defect.objects.all():
						if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Elaboration Phase' and instance2.iteration_name == 'Iteration 1':
							count=count+1
				if request.session['phase_name'] == 'Construction Phase' and request.session['iteration_name'] == 'Iteration 1':
					for instance2 in Defect.objects.all():
						if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Elaboration Phase' and instance2.iteration_name == 'Iteration 2':
							count=count+1
				if request.session['phase_name'] == 'Construction Phase' and request.session['iteration_name'] == 'Iteration 2':
					for instance2 in Defect.objects.all():
						if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Construction Phase' and instance2.iteration_name == 'Iteration 1':
							count=count+1
				if request.session['phase_name'] == 'Construction Phase' and request.session['iteration_name'] == 'Iteration 3':
					for instance2 in Defect.objects.all():
						if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Construction Phase' and instance2.iteration_name == 'Iteration 2':
							count=count+1
				if request.session['phase_name'] == 'Transition Phase' and request.session['iteration_name'] == 'Iteration 1':
					for instance2 in Defect.objects.all():
						if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Construction Phase' and instance2.iteration_name == 'Iteration 3':
							count=count+1
				if request.session['phase_name'] == 'Transition Phase' and request.session['iteration_name'] == 'Iteration 2':
					for instance2 in Defect.objects.all():
						if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Transition Phase' and instance2.iteration_name == 'Iteration 1':
							count=count+1
				twentypercent = 0.2*count
				instance.twentypercent=twentypercent

				instance.save()


	if 'metrics' in request.POST:
		return iterationviewmetrics(request)
	context = {
		"state": request.session['state'],
		"open": "open",
		"close": "close",
		...
		try:
			...
			if (minstance.is_open==False):
				...
				try:
					...
					return render(request, "developmentEdit.html", context)
				except Activity.DoesNotExist:
		...
		try:
			...
			if (minstance.is_open==False):
				...
				try:
					...
					return render(request, "defectsEdit.html", context)
				except Activity.DoesNotExist:
...
def phaseviewmetrics(request):
	...
	for obj in activities:
		if obj.project_name == project_name and obj.phase_name == phase_name:
			...
			if obj.activity_type == 'Development':
				...
				removals = removals + int(obj.defects)
	count=0
	for obj in Defect.objects.all():
		if obj.project_name == project_name and obj.phase_name == phase_name and iteration_name == iteration_name:
			count=count+1
	count2=0
	countinception=0
	twentypercent=0.0
	if request.session['phase_name'] == 'Elaboration Phase':
		for instance2 in Iteration.objects.all():
			if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Inception Phase':
				countinception=countinception+1
	if request.session['phase_name'] == 'Elaboration Phase':
		for instance2 in Iteration.objects.all():
			if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Inception Phase' and instance2.iteration_name=='Iteration 1':
				countinception=countinception+1
			if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Elaboration Phase':
				count2=count2+instance2.twentypercent
	if request.session['phase_name'] == 'Construction Phase':
		for instance2 in Iteration.objects.all():
			if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Inception Phase' and instance2.iteration_name=='Iteration 1':
				count2=count2+instance2.twentypercent
			if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Elaboration Phase':
	...
	if request.session['phase_name'] == 'Construction Phase':
		for instance2 in Iteration.objects.all():
			...
			if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Elaboration Phase':
				count2=count2+instance2.twentypercent
			if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Construction Phase':
				count2=count2+instance2.twentypercent
	if request.session['phase_name'] == 'Construction Phase':
		for instance2 in Iteration.objects.all():
			if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Inception Phase':
			...
			if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Construction Phase':
				count2=count2+instance2.twentypercent
	if request.session['phase_name'] == 'Transition Phase':
		for instance2 in Iteration.objects.all():
			if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Inception Phase':
				count2=count2+instance2.twentypercent
			if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Elaboration Phase':
	...
	if request.session['phase_name'] == 'Transition Phase':
		for instance2 in Iteration.objects.all():
			...
			if instance2.project_name == request.session['project_name'] and instance2.phase_name == 'Transition Phase' and instance2.iteration_name == 'Iteration 1':
				count2=count2+instance2.twentypercent
	twentypercent = 0.2*count2
	injections=count+twentypercent
...
def iterationviewmetrics(request):
	...
	for obj in Defect.objects.all():
		if obj.project_name == project_name and obj.phase_name == phase_name and iteration_name == iteration_name:
			count=count+1
	#injections = count + estimate_yield*count/100
	#removals = count
	#print(count)
	count2=0
	twentypercent=0
